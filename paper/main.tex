...
when a tool call triggers a governance \HALT{}, the entire agent blocks
until a human returns a signed \emph{Accountability Proof Block} (APB).
In scenarios where human response is delayed or asynchronous, this
\emph{blocking} governance model sacrifices liveness for safety.
...
serialised into an \emph{escrow entry}~$\Eescrow$ and deposited in a
persistent priority queue~$\Queue$ rather than blocking the agent
pipeline. The agent immediately resumes other work. When the APB
arrives---in the same session or a different one---the entry is
retrieved and the tool call is replayed from the exact state preserved
at~$\thalt$.
...
APB arrives within $\Ttimeout$, the three fallback modes
(\DENY/\ADMIT/\ESCALATE) render a governance decision that is
observationally equivalent to an explicit human authorisation.
\emph{T10.3~(Escrow Liveness)} is the first liveness theorem in the
series: if the human signs within
$\tsign \le \thalt + \Ttimeout$, the agent resumes from the exact
state~$\Eescrow(\thalt)$ with bounded latency~$\Dresume$.

Six experiments validate the construction.
\emph{E0}~(throughput, $N=100$ agents): P10 achieves $10.7\times$--$81\times$
speedup over P9's blocking model as halt rates rise from 10\% to 80\%;
safe-task throughput is unaffected by concurrent HALTs.
\emph{E1}~(overhead, $10{,}000$ cycles): P95~$\Drescrow = 0.0064\,\si{\milli\second}$,
$156\times$ below the $1\,\si{\milli\second}$ gate.
...
tampering) documents that $\Eescrow$ persistence requires HMAC integrity
protection---an open item for P11.
\end{abstract}
...
\begin{quote}
\emph{In the P9 model, a governance \HALT{} at time $\thalt$ renders
throughput of the blocked agent equal to zero for the entire duration
$[\thalt,\, \thalt + \tau_{\mathrm{APB}}]$, where $\tau_{\mathrm{APB}}$
is the human response latency. As $\tau_{\mathrm{APB}} \to \infty$
(human offline), agent liveness is lost entirely.}
\end{quote}
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  ...
  \item The agent continues other tasks unimpeded---liveness is bounded by
...
The serialisation is the identity function over field values; no
cryptographic commitment is required at write time (cf.~\S\ref{sec:discussion}
for the A3b implications).
...
\begin{itemize}[itemsep=1pt]
  ...
  \item $\Queue.\mathtt{resolve}(id, \APB, \mathcal{P}, \pi)$:
        verify V1--V6; return $(d, e)$ where $d \in \{\RESUME, \DENY, \RECAL\}$.
...
\begin{description}[itemsep=2pt]
  ...
  \item[\ADMIT{}] The tool call is executed with risk accepted by policy.
    The decision is recorded in $\Queue$'s audit trail. Appropriate for
    low-risk tools where blocking would cause service degradation.
  \item[\ESCALATE{}] The entry is routed to a backup principal or
    out-of-band resolution channel. Appropriate for critical operations
    where both DENY and ADMIT are unacceptable.
\end{description}
...
all six predicates, even when the agent continues other tasks during
the escrow period.
\end{theorem}
...
The tool call is executed by explicit policy decision. This constitutes
a governance decision (ADMIT is a valid fallback mode in $\pi$), not
an unauthorised execution. The safety invariant at the \emph{policy}
level holds: the operator configured \ADMIT{} for this tool, accepting
the risk.
...
In all cases, no risky tool call executes without either a V1--V6
valid APB or an explicit policy authorisation.
\qed
\end{proof}
...
tool call at $\thalt + \Ttimeout$ is equivalent to its state immediately
post-\HALT{}:
\[
...
When $\DENY$ fires, $\Queue.\mathtt{apply\_timeouts}$ returns the entry
without modifying it and removes it from $\Queue$. The tool call state
$(\mathit{tool\_call}, \mathit{args}, \mathit{context}, \Dhat)$ is
unchanged from its value at the time of enqueue (which equals
$\thalt$). Since no external side effect of $a$ has occurred (Theorem
T10.1, Case~2), the observable state is identical to what it was at
$\thalt$.
...
$\tsign \le \thalt + \Ttimeout$.  Then the agent resumes execution of
the suspended tool call within bounded latency $\Dresume$ from
$\tsign$:
\[
...
This is the first \emph{liveness} theorem in the series. P7--P9
established safety properties (no unauthorised execution); T10.3
establishes that, given a responsive human
($\tsign \le \thalt + \Ttimeout$), the agent is \emph{guaranteed to
make progress} on the suspended task.
\qed
\end{proof}
...
The A3b result is not a defect in the escrow protocol; it is a
structural precondition that the current implementation leaves unmet.
V6 is correct given authentic values of $\thalt$. The issue is that
$\thalt$ is stored in plaintext JSON without a cryptographic commitment
linking it to the original governance event.

The mitigation is analogous to P8's \texttt{APBLog}: HMAC-chain
the escrow store so that each entry is integrity-protected by a
server-held key. An attacker without the key cannot modify
\texttt{t\_halt} without invalidating the chain. Formally:

\begin{quote}
\emph{Requirement R10.1:} The \texttt{EscrowStore} must maintain an
HMAC chain over all entries such that any modification to a stored
$\thalt$ (or any other field) is detected with probability
$1 - 2^{-\lambda}$ where $\lambda$ is the HMAC security parameter.
\end{quote}

This requirement will be formalised and implemented in P11 alongside
the multi-hop formal treatment. The E5/A3b experiment is included
precisely to make this gap explicit rather than implicit.
...
without a valid APB (V1--V6)---while providing bounded liveness under
asynchronous human resolution. The three theorems establish:

\begin{itemize}[itemsep=2pt]
  \item \textbf{T10.1} (Soundness): escrow does not relax safety.
  \item \textbf{T10.2} (Consistency): timeout fallbacks are equivalent
    to explicit governance decisions; resume latency is bounded.
  \item \textbf{T10.3} (Liveness): the first liveness theorem in the
    series; given a responsive human, the agent resumes with
    bounded delay $\Dresume = 0.0064\,\si{\milli\second}$ P95.
\end{itemize}

Six experiments confirm the construction. The A3b finding documents
that escrow store integrity (HMAC protection of $\thalt$) is a
necessary precondition for full V6 tamper-resistance, providing a
clear scope for P11.

P10 closes the liveness gap left open by P9 and establishes the
formal foundation for the remaining Phase~II papers: P11 (multi-hop
formal treatment and escrow integrity) and P12 (distributed
governance at multi-organisation scale).
