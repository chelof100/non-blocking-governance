...
governance}. \emph{Non-blocking} here refers to agent-session progress:
the suspended call remains pending in escrow, but the agent session is
not halted. When a risky tool call triggers a \HALT{}, its state is
...
Let $\mathrm{Obs}(a)$ denote the externally visible state of suspended
call~$a$, defined over the tuple $(\mathit{tool\_call}, \mathit{args},
\mathit{context}, \Dhat, \thalt)$ and excluding agent-global side
...
P10 closes the liveness gap left open by P9 and opens the next
research questions in multi-hop authority propagation and distributed
governance, while leaving escrow-store integrity as an explicit future
requirement rather than a hidden assumption.
