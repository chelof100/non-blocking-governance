...
when a tool call triggers a governance \HALT{}, the suspended tool call
...
governance}. \emph{Non-blocking} refers to continued agent progress: the suspended
tool call remains in escrow while the agent session continues.
When a risky tool call triggers a \HALT{}, its state is
...
unrelated work while the suspended tool call remains in escrow. When the
...
\emph{E0}~(throughput, $N=100$ agents): P10 achieves up to
$\mathbf{81\times}$ higher throughput than P9 at 80\% halt rate
($10.7\times$ at 10\%); safe-task throughput remains unaffected in
all cases.
\emph{E1}~(overhead, $10{,}000$ cycles):
P95~$\Drescrow = \mathbf{6.4\,\si{\micro\second}}$, $156\times$ below
the $1\,\si{\milli\second}$ gate.
...
protection---an implementation gap left as future work.
\end{abstract}
...
\noindent Fallback modes do not weaken safety: they externalise the
authorisation decision from the human principal to a pre-committed
governance policy. The safety invariant of T10.1 holds under all three
modes.
...
\Comment{amortised $O(1)$ in-memory; returns immediately}
...
The suspended call executes only because the configured policy
...
In all cases, the suspended call does not execute without either
...
\begin{remark}
$\mathrm{Obs}(a)$ denotes the externally visible state of suspended
call~$a$: the tuple $(\mathit{tool\_call}, \mathit{args},
\mathit{context}, \Dhat, \thalt)$, excluding agent-global side
effects such as memory updates and other tool calls in flight.
\end{remark}
...
\emph{suspended call, considered in isolation}, is equivalent at
...
$\Delta_{\mathrm{get}}$). Both are amortised $O(1)$ in the size of the
entry (constant-width fields). E1 measures the serialisation roundtrip
$\Drescrow = 0.0064\,\si{\milli\second}$ P95, which serves as the
conservative upper bound on $\Dresume$. The in-memory resume path
(without file I/O) measures $0.0001\,\si{\milli\second}$ P95 (E0),
confirming that $\Dresume$ is bounded and negligible relative to LLM
inference and human response times.
\qed
\end{proof}
...
$\tsign \le \thalt + \Ttimeout$.  Then the suspended call resumes
...
$\Dverify + \Delta_{\mathrm{get}} \le \Dresume \le \Drescrow$, where
$\Drescrow = 0.0064\,\si{\milli\second}$ P95 is the serialisation
upper bound from E1; the tighter in-memory path measures
$\Dresume = 0.0001\,\si{\milli\second}$ P95 (E0).
...
The escrow layer is implemented as an additive extension over the
frozen P7--P9 stack (\texttt{stack/}, \texttt{proxy/}, \texttt{agent/});
no existing file is modified.

\subsection{Usage Example}

Integrating non-blocking governance requires only minimal
configuration:
...
Between \texttt{enqueue} and \texttt{resolve}, the agent session is
not blocked: it continues other tasks while the suspended call remains
in escrow. No changes to tool servers or core agent logic are required.
...
$r \in \{0, 0.1, 0.2, 0.4, 0.6, 0.8\}$. E0 evaluates
\emph{protocol-level} throughput under simulated human latency; it is
a logical-time simulation, not a wall-clock OS-level benchmark.
Escrow overhead uses measured $\Drescrow$ and $\Dresume$ P95 values.

\paragraph{Two metrics (measured separately).}
\begin{itemize}[itemsep=2pt]
  \item \textbf{Metric 1} ($\Dresume$): latency from APB arrival to
    task resumption via the in-memory queue path; P95
    $= 0.0001\,\si{\milli\second}$ (measured in E0). The upper bound
    including serialisation is $\Drescrow = 0.0064\,\si{\milli\second}$
    P95 (E1).
...
which approaches $50\,\text{tps}$ as $r \to 1$ (by construction, 50\% of
tasks are governance-free baseline tasks; none block). P9 throughput degrades as
...
This requirement is left as an implementation gap for future work.
We include A3b precisely to make it explicit rather than implicit.
...
\begin{itemize}[itemsep=2pt]
  ...
  \item \textbf{T10.3} (Liveness): the first liveness theorem in the
    ...
    call resumes with bounded delay $\Dresume \le 0.0064\,\si{\milli\second}$
    P95 (conservative bound from E1; in-memory path $0.0001\,\si{\milli\second}$, E0).
\end{itemize}
...
production. The A3b finding makes this gap explicit.

P10 closes the liveness gap left open by P9 and sets up P11's
escrow-store integrity (R10.1) and multi-hop formal treatment, while
leaving those extensions as explicit future requirements rather than
hidden assumptions. \emph{P9 made governance safe but blocking.
P10 shows it can be safe and live.}
